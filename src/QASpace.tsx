...
import {
  ...
  storiesForProduct, storyHealth, epicHealth, releaseHealth, releasePct, workflowTasksForSpace, acknowledgeWorkflowTask, completeWorkflowTask,
            ...
            <div className="flex items-center justify-center py-4">
              <div className="relative w-28 h-28">
                <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                  ...
                  <circle cx="50" cy="50" r="38" fill="none" stroke="#4F46E5" strokeWidth="10" strokeDasharray={`${2 * Math.PI * 38 * (releasePct(rel) / 100)} ${2 * Math.PI * 38}`} strokeLinecap="round" />
                </svg>
      ...
      <div className="flex gap-4 overflow-x-auto px-6 py-5 flex-1">
        {columns.map(col => (
          <div key={col.id} className="flex-shrink-0 w-72 flex flex-col gap-3">
            <div className="flex items-center justify-between"><span className="text-[12px] font-semibold text-[#555]">{col.label}</span><span className="text-[10px] bg-[#EBEBEB] text-[#555] px-1.5 py-0.5 rounded-full">{col.stories.length}</span></div>
            ...
            {/* Same dashed-box "Empty" treatment as the shared KanbanBoard's empty columns (ui.tsx) —
                this is structurally the same thing (an empty column in a card board), so it shouldn't
                read as a different, bespoke empty state just because this board is hand-rolled. */}
    ...
    <WorkspaceShell title="Test Cases" subtitle={`${TEST_CASES.length} total across active stories`} actions={<Btn variant="primary" onClick={() => navigate('create-test-case')}>+ Test Case</Btn>}>
      <div className="flex items-center gap-2 mb-4">
        {(['all', 'Failed', 'Blocked', 'Passed'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)} className={`text-[11.5px] px-3 py-1.5 rounded-full border transition-colors ${filter === f ? 'bg-[#4F46E5] text-white border-[#4F46E5]' : 'border-[#E0E0E0] text-[#666] hover:bg-[#F5F5F5]'}`}>{f === 'all' ? 'All' : f}</button>
        ))}
      </div>
      <CardShell>
    ...
    <WorkspaceShell title="Bugs" subtitle={`${BUGS.filter(b => b.workflowState === 'Open').length} open · ${BUGS.filter(b => b.severity === 'Critical').length} critical`} actions={<Btn variant="primary" small onClick={() => navigate('create-bug')}>+ Log Bug</Btn>}>
      <div className="flex items-center gap-2 mb-4">
        {[{ id: 'open', l: 'Open' }, { id: 'fixed', l: 'Fixed / Verified' }, { id: 'all', l: 'All' }].map(f => (
          <button key={f.id} onClick={() => setFilter(f.id)} className={`text-[11.5px] px-3 py-1.5 rounded-full border transition-colors ${filter === f.id ? 'bg-[#4F46E5] text-white border-[#4F46E5]' : 'border-[#E0E0E0] text-[#666] hover:bg-[#F5F5F5]'}`}>{f.l}</button>
        ))}
      </div>
      <CardShell>
  ...
  return (
    <WorkspaceShell title="Release Readiness" subtitle="Gate-by-gate sign-off for the current release cycle" actions={<><Btn small variant="outline">Request Fix</Btn><Btn variant={gatePct === 100 ? 'primary' : 'outline'} small onClick={gatePct === 100 ? () => approveRelease(rel.id) : undefined}>{gatePct === 100 ? 'Approve Release' : `Release Blocked (${rel.gateChecks.filter(g => !g.passed).length} open)`}</Btn></>}>
      <div className="flex items-center gap-2 mb-5">
        {RELEASES.map(r => (
          <button key={r.id} onClick={() => setSelected(r.id)} className={`text-[11.5px] px-3 py-1.5 rounded-full border transition-colors ${selected === r.id ? 'bg-[#4F46E5] text-white border-[#4F46E5]' : 'border-[#E0E0E0] text-[#666] hover:bg-[#F5F5F5]'}`}>{r.name} · {r.targetDate}</button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_280px] gap-5">
            ...
            {epics.map(epic => {
              ...
              return (
                <div key={epic.id} className="px-4 py-3 border-b border-[#F5F5F5] last:border-0">
                  ...
                  <ProgressBar pct={stories.length ? Math.round(approvedCount / stories.length * 100) : 0} health={epicHealth(epic)} />
          ...
          <CardShell className="p-4">
            ...
            {[{ name: 'Auth flows', passed: 24, total: 24, pct: 100 }, { name: 'Checkout path', passed: 18, total: 30, pct: 60 }, { name: 'Profile & settings', passed: 12, total: 12, pct: 100 }, { name: 'Push & permissions', passed: 0, total: 8, pct: 0 }].map(s => (
              <div key={s.name} className="flex items-center justify-between py-2.5 border-b border-[#F5F5F5] last:border-0">
                ...
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded flex-shrink-0 ${s.pct === 100 ? 'bg-[#F0F0F0] text-[#555]' : s.pct === 0 ? 'bg-[#FAFAFA] text-[#BBBBBB] border border-dashed border-[#DDDDDD]' : 'bg-[#EBEBEB] text-[#555]'}`}>{s.pct === 100 ? 'Complete' : s.pct === 0 ? 'Not started' : 'Partial'}</span>
              </div>
            ))}
          </CardShell>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <CardShell className="p-4">
            ...
            <div className="mb-4"><div className="flex items-center justify-between text-[11px] mb-1.5"><span className="text-[#888]">Overall readiness</span><span className="font-medium text-[#333]">{rel.gateChecks.filter(g => g.passed).length}/{rel.gateChecks.length} passed</span></div><ProgressBar pct={gatePct} health={releaseHealth(rel)} /></div>
